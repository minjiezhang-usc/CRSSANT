...
def spec_range(start,end,interval):
    out_list = []
    if int(start) == int(end): out_list = [int(start)]
    if int(start) < int(end):  out_list = [i for i in range(int(start), int(end), int(interval))]
    return out_list
...
def getcov(bedgraphplus,bedgraphminus):
    ...
    for line in f: #plus strand
        ...
        range10nt = spec_range(round(int(start)/10),round(int(end)/10),1)
...
def create_stats(dg_align_dict, genealign, covdict):
    ...
    for (dg, alignids) in dg_align_dict.items():
        ...
        range1=[i*10 for i in spec_range(floor(int(start1)/10),ceil(int(end1)/10),1)]
        range2=[i*10 for i in spec_range(floor(int(start2)/10),ceil(int(end2)/10),1)]
...
print(timenow()+'\t'+"Started DGs assembly"+'\n')
def parse_args():
